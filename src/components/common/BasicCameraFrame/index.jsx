...
export const BasicCameraFrame2 = ({ bgsrc, isCapturing, currentFrame }) => {
  ...
  return (
    <div
      className={`flex h-[450px] w-[350px] flex-col gap-[4px] border-2 px-[13px] py-[26px] shadow-lg ${
        isColor ? `bg-BGColor-${bgsrc}` : ""
      }`}
      style={{
        backgroundImage: isImage ? `url(${bgsrc})` : "none",
          ...
          >
            {frames[0] ? (
              <img
                src={frames[0]}
                alt="Frame 1"
                className="h-[180px] w-[160px] object-cover"
              />
            ) : (
              <video
...
export const BasicCameraFrame3 = ({ bgsrc, isCapturing, currentFrame }) => {
  ...
  return (
    <div
      className={`w-real-screen flex h-[320px] flex-col gap-[5px] border-2 px-[23px] py-[17px] shadow-lg ${
        isColor ? `bg-BGColor-${bgsrc}` : ""
      }`}
      style={{
        backgroundImage: isImage ? `url(${bgsrc})` : "none",
          ...
          <div className="border-2 bg-white" key={index}>
            {frames[index] ? (
              <img
                src={frames[index]}
                alt={`Frame ${index + 1}`}
                className="h-[134px] w-[164px] object-cover"
              />
            ) : (
              <video
                ref={videoRefs.current[index]}
...
export const BasicCameraFrame4 = ({ bgsrc, isCapturing, currentFrame }) => {
  ...
  return (
    <div
      className={`w-real-screen flex h-[320px] flex-col gap-[10px] border-2 px-[23px] py-[17px] shadow-lg ${
        isColor ? `bg-BGColor-${bgsrc}` : ""
      }`}
      style={{
        backgroundImage: isImage ? `url(${bgsrc})` : "none",
          ...
          <div className="border-2 bg-white" key={index}>
            {frames[index] ? (
              <img
                src={frames[index]}
                alt={`Frame ${index + 1}`}
                className="h-[132px] w-[170px] object-cover"
              />
            ) : (
              <video
                ref={videoRefs.current[index]}
